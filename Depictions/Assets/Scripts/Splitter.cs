...
public class Splitter : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (activated)
        {
            ...
            if (Physics.Raycast(transform.position + (transform.forward / 2), transform.parent.TransformDirection(Vector3.right), out hit, Mathf.Infinity))
            {
                Debug.DrawLine(transform.position + (transform.forward / 2), hit.transform.position, Color.yellow);
                //If we hit a mirror, activate that mirror.
                if (hit.transform.gameObject.layer == 9)
            ...
            if (Physics.Raycast(transform.position + (transform.forward / 2), transform.parent.TransformDirection(Vector3.left), out hit, Mathf.Infinity))
            {
                Debug.DrawLine(transform.position + (transform.forward / 2), hit.transform.position, Color.yellow);
                //If we hit a mirror, activate that mirror.
                if (hit.transform.gameObject.layer == 9)
